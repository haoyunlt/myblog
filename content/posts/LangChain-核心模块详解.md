...
class BaseMessage(BaseModel, ABC):
    ...
    content: Union[str, List[Union[str, Dict]]] = Field(default="")
    """消息内容，支持文本和多模态内容"""
    ...
    def __add__(self, other: "BaseMessage") -> "BaseMessageChunk":
        """消息合并操作"""
        if not isinstance(other, BaseMessage):
            raise TypeError(f"Cannot add {type(other)} to BaseMessage")
        
        # 创建合并后的消息块
        return BaseMessageChunk(
            content=self._merge_content(self.content, other.content),
            additional_kwargs=self._merge_kwargs(
                self.additional_kwargs, 
                other.additional_kwargs
            ),
            response_metadata=self._merge_metadata(
    ...
    def _merge_content(self, left: Any, right: Any) -> Any:
        """合并消息内容"""
    ...
    def _merge_kwargs(self, left: Dict, right: Dict) -> Dict:
        """合并额外参数"""
        merged = left.copy()
        merged.update(right)
        return merged
...
def convert_to_messages(
    message_like: List[Union[BaseMessage, Dict[str, Any], str, Tuple[str, str]]]
) -> List[BaseMessage]:
    """
    将各种格式转换为标准消息列表
