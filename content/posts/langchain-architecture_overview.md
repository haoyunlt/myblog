...
tags: ["LangChain", "架构设计", "Runnable", "LCEL", "AI应用", "源码分析"]
categories: ["langchain", "AI框架"]
author: "LangChain架构分析"
description: "深入剖析LangChain框架的整体架构设计、核心组件和设计哲学，从统一抽象到高效组合的完整解决方案"
toc: true
tocOpen: true
showReadingTime: true
showWordCount: true
weight: 220
slug: "langchain-architecture_overview"
---

## 概述

LangChain是一个用于构建大语言模型(LLM)应用的框架，其核心设计理念是通过统一的抽象接口和声明式组合语法，让开发者能够快速构建复杂的AI应用。LangChain的架构设计，揭示其背后的技术细节和设计哲学。
...
- `invoke()` → 子类实现
- `|` 组合：`Runnable.__or__()` → `RunnableSequence`

类结构图（Mermaid）：

```mermaid
classDiagram
    class Runnable { <<abstract>> +invoke() }
    class RunnableSequence { +invoke() +batch() +stream() }
    Runnable <|-- RunnableSequence
```

时序图索引：见“5.1 Chain执行时序图”。

### 10.2 语言模型抽象（BaseLanguageModel / BaseChatModel）
...
class BaseLanguageModel(RunnableSerializable[LanguageModelInput, BaseMessage], ABC):
    def generate(self, messages: List[List[BaseMessage]], stop: Optional[List[str]] = None, callbacks: Callbacks = None, **kwargs) -> LLMResult:
        """按消息序列生成结果，触发回调并聚合 generations 与 llm_output。"""
        ...
...
class BaseChatModel(BaseLanguageModel):
    def _generate(self, messages: List[BaseMessage], stop: Optional[List[str]] = None, run_manager: Optional[CallbackManagerForLLMRun] = None, **kwargs) -> ChatResult:
        """子类实现的核心生成方法；负责单轮消息生成。"""
        ...
```

统一调用链：

- `generate()` → 回调 on_llm_start → `_generate()` → on_llm_end
...
时序图索引：见“4.1 语言模型抽象”图。

### 10.3 向量存储抽象（VectorStore）
...
- 写入：`add_texts()` → 生成嵌入 → 写入索引
- 检索：`similarity_search_with_score()` → 嵌入查询 → 向量库搜索

类结构图（Mermaid）：

```mermaid
classDiagram
    class VectorStore { <<abstract>> +add_texts() +similarity_search() }
```

时序图索引：见“4.2 向量存储抽象”。

### 10.4 工具与回调（BaseTool / CallbackManager）
...
classDiagram
    ...
    class CallbackManager { +on_chain_start() +on_llm_start() }
```
